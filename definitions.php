...
function getDefinitions($table) {
    ...
    if ($currentUser['role'] === 'branch_manager' || $currentUser['role'] === 'technician') {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    branchModal.addEventListener('show.bs.modal', function() {
        ...
        if (action === 'add') {
            // Load company city as default
            fetch('definitions.php?action=get_company_info')
                .then(response => response.json())
                .then(data => {
                    if (data.success && data.company && data.company.city) {
                        citySelect.value = data.company.city;
                        loadDistricts(data.company.city);
                    }
                });
        }
    });
