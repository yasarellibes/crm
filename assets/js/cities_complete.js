/**
 * TÜRKIYE İL/İLÇE COMPLETE SYSTEM - TurkiyeAPI.dev
 * 81 İL - 973 İLÇE - GERÇEK VERİ
 */
...
const TURKEY_CITIES = {
    ...
    "Elazığ": ["Ağın", "Alacakaya", "Arıcak", "Baskil", "Karakoçan", "Keban", "Kovancilar", "Maden", "Merkez", "Palu", "Sivrice"],
    ...
    "Tekirdağ": ["Çerkezköy", "Çorlu", "Ergene", "Hayrabolu", "Kapaklı", "Malkara", "Marmaraereğlisi", "Murefte", "Saray", "Süleymanpaşa", "Şarköy"],
...
// Global Setup Fonksiyonu
function setupCityDistrict(citySelectId, districtSelectId, defaultCity = '', defaultDistrict = '') {
    console.log('🚀 setupCityDistrict başlatıldı:', { citySelectId, districtSelectId, defaultCity, defaultDistrict });
    ...
    if (!citySelect) {
        console.error('❌ İl dropdown bulunamadı:', citySelectId);
        return false;
    }
    
    if (!districtSelect) {
        console.error('❌ İlçe dropdown bulunamadı:', districtSelectId);
        return false;
    }
    
    console.log('✅ Dropdownlar bulundu');
    ...
    console.log('✅ İller yüklendi:', Object.keys(TURKEY_CITIES).length);
    
    // İlçeleri yükle (eğer varsayılan il varsa)
    if (defaultCity && TURKEY_CITIES[defaultCity]) {
        districtSelect.innerHTML = '<option value="">İlçe Seçiniz</option>';
        TURKEY_CITIES[defaultCity].forEach(district => {
            const option = document.createElement('option');
        ...
        console.log('✅ İlçeler yüklendi:', TURKEY_CITIES[defaultCity].length, 'adet');
    } else {
        districtSelect.innerHTML = '<option value="">İlçe Seçiniz</option>';
        districtSelect.disabled = true;
    }
    
    // İl değişikliği event listener
    citySelect.addEventListener('change', function() {
        const selectedCity = this.value;
        console.log('🔄 İl değişti:', selectedCity);
        ...
        if (selectedCity && TURKEY_CITIES[selectedCity]) {
            ...
            districtSelect.disabled = false;
            console.log('✅ Yeni ilçeler yüklendi:', TURKEY_CITIES[selectedCity].length, 'adet');
        } else {
            districtSelect.innerHTML = '<option value="">İlçe Seçiniz</option>';
            districtSelect.disabled = true;
            console.log('ℹ️ İlçe dropdown devre dışı');
        }
    });
    
    console.log('🎉 setupCityDistrict tamamlandı - 81 il, 973 ilçe hazır!');
    return true;
}

// Window nesnesine ekle
window.setupCityDistrict = setupCityDistrict;

// Sayfa yüklendiğinde bilgi ver
document.addEventListener('DOMContentLoaded', function() {
    console.log('🇹🇷 Türkiye İl/İlçe sistemi hazır - TurkiyeAPI.dev verisi');
    console.log('📊 81 İl, 973 İlçe yüklendi');
    console.log('🔧 Kullanım: setupCityDistrict("cityId", "districtId", "defaultCity", "defaultDistrict")');
});